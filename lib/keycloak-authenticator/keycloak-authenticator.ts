...
function buildConfig() {
  return {
    ...
    'auth-server-url': (process.env.KEYCLOAK_AUTH_SERVER_URL as string) || 'http://13.127.63.157:8080/auth',
    'ssl-required': (process.env.KEYCLOAK_SSL_REQUIRED as string) || 'external',
    'use-resource-role-mappings': (process.env.KEYCLOAK_USE_RESOURCE_ROLE_MAPPINGS as string) || 'false',
    ...
    'bearer-only': (process.env.KEYCLOAK_BEARER_ONLY as string) || 'true',
